...
const SORT_MAP: Record<string, { column: string; ascending: boolean }> = {
	...
	price_asc: { column: 'price_flat', ascending: true },
	price_desc: { column: 'price_flat', ascending: false },
...
venuesRouter.get('/', zValidator('query', listQuerySchema), async (c) => {
	...
	let query = supabase
		.from('venues')
		.select(
			'id, slug, name, short_description, location, district, capacity_min, capacity_max, price_flat, rating, review_count, image_url, images, is_featured, is_new, status, created_at, categories(id, slug, name)',
			{ count: 'exact' },
		)
	...
	if (minPrice != null) query = query.gte('price_flat', minPrice);
	if (maxPrice != null) query = query.lte('price_flat', maxPrice);
...
venuesRouter.post(
	...
	authenticate,
	requireProvider,
	zValidator('json', createVenueBodySchema),
	async (c) => {
	...
	async (c) => {
		...
		const [{ data: event_packages }, { data: provider_services }] = await Promise.all([
			supabase
				...
				.select(
					`id, venue_id, slug, name, short_description, price_per_person, guests_min, guests_max, sort_order, is_active, image_url, images, created_at, updated_at, venue_package_services (${packageServiceSelect})`,
				)
				.eq('venue_id', venueId)
				.order('sort_order', { ascending: true }),
			supabase
				.from('provider_services')
				.select('id, slug, name, kind, price_flat, status, image_url, contact_phone, contact_email, website, sort_order')
...
venuesRouter.patch(
	...
	async (c) => {
		...
		if (
			'amenities' in updates &&
			Array.isArray(updates.amenities) &&
			(updates.amenities as string[]).length === 0
		) {
		...
		if (hasPackageUpdates) {
			...
			const { data: pkgs } = await supabase
				.from('venue_event_packages')
				.select(
					'id, venue_id, slug, name, short_description, price_per_person, guests_min, guests_max, sort_order, is_active, image_url, images, created_at, updated_at, venue_package_services (*)',
				)
				.eq('venue_id', venueId)
				.order('sort_order', { ascending: true });
